...
def predict_image_path(image_path, out_dir=None, stem_override=None):
    ...
    if inference_module is not None and hasattr(inference_module, 'infer_image'):
        try:
            ...
            return {'image': image_path, 'pred_mask': pred_mask, 'saved_dir': str(out_dir) if out_dir else None, 'stem': stem_override or image_path.stem}
        except Exception:
            pass
    ...
    return {'image': image_path, 'pred_prob': pred, 'pred_mask': pred_mask, 'saved_dir': str(out_dir) if out_dir else None, 'stem': stem}
...
def run_on_dir(img_dir, mask_dir=None, results_out_dir=None, skip_empty_gt=True):
        ...
        if mask_dir:
            ...
            if mask_path:
                ...
                if skip_empty_gt and gt_sum == 0:
                    per_image.append({'image': p.name, 'dice': None, 'iou': None, 'gt_sum': 0, 'pred_sum': int(pred_mask.sum())})
                    continue
    ...
    import numpy as _np
    return {'per_image': per_image, 'mean_dice': float(_np.mean(dices)) if dices else None, 'mean_iou': float(_np.mean(ious)) if ious else None}
